...
class FocusFilterApplication : Application() {
    ...
    private fun maybePostReminderNotification() {
        ...
        // Only remind if there are actually inbox items to review
        appScope.launch(Dispatchers.IO) {
            val hasInboxItems = database.filteredNotificationDao()
                .getInboxItems()
                .let { flow ->
                    var result = false
            ...
            if (!hasInboxItems) return@launch

            postReminderNotification(prefs)
        }
    }

    private fun postReminderNotification(prefs: android.content.SharedPreferences) {
